...
class query_utils {
 public:
  ...
  static filter_query expression_to_filter_query(packet_store::handle* h,
    ...
    typedef filter_query::iterator iterator_t;
 ...
 private:
  static bool reduce_conjunction(filter_conjunction& conj) {
    typedef filter_conjunction::iterator iterator_t;
  ...
  static basic_filter predicate_to_basic_filter(packet_store::handle* h,
      predicate* p) {
  ...
  static basic_filter ip_filter(uint32_t index_id, const std::string& op,
                                      const std::string& ip_string) {
    if (op == "in") {
      auto range = ip_range(ip_string);
      return basic_filter(index_id, range.first, range.second);
    } else if (op == "!in") {
      auto range = ip_range(ip_string);
      return basic_filter(index_id, range.first, range.second);
    } else if (op == "==") {
      uint32_t ip = ip_string_to_uint32(ip_string.c_str());
      return basic_filter(index_id, ip, ip);
    } else if (op == "!=") {
      uint32_t ip = ip_string_to_uint32(ip_string.c_str());
      return basic_filter(index_id, ip, ip);
    } else {
  ...
  static basic_filter port_filter(uint32_t index_id, const std::string& op,
    ...
    if (op == "==") {
      return basic_filter(index_id, port, port);
    } else if (op == "!=") {
      return basic_filter(index_id, port, port);
    } else if (op == "<") {
      return basic_filter(index_id, 0, port - 1);
    } else if (op == "<=") {
      return basic_filter(index_id, 0, port);
    } else if (op == ">") {
      return basic_filter(index_id, port + 1, UINT16_MAX);
    } else if (op == ">=") {
      return basic_filter(index_id, port, UINT16_MAX);
    } else {
      throw parse_exception("Specify port ranges with <,>,<=,>= operators");
    }
  }

  static basic_filter time_filter(uint32_t index_id, const std::string& op,
